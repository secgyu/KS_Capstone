...
function CheckBox({ children, icon, isChecked = false, ...props }: CheckBoxProps) {
  ...
  return (
    <Pressable style={({ pressed }) => [pressed && styles.optionButtonPressed, styles.checkBoxContainer]} {...props}>
      <Ionicons name={`checkmark-circle${isChecked ? "" : "-outline"}`} size={22} color={colors[theme].BLUE_500} />
      {icon}
