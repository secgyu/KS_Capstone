...
function YearSelector({ isVisible, currentyear, onChangeYear, hide }: YearSelectorProps) {
  ...
  useEffect(() => {
    ...
    const currentRow = Math.floor(yearIndex / numbers.CALENDAR_YEAR_SELECTOR_COLUMN);
      ...
      {isVisible && (
        <View style={styles.container}>
          <View style={styles.yearsContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={[styles.yearText, currentyear === item.num && styles.currentYearText]}>{item.num}</Text>
                </Pressable>
              )}
          ...
          <Pressable style={styles.closeButton} onPress={hide}>
            ...
            <MaterialIcons name="keyboard-arrow-up" size={20} color={colors[theme].BLACK} />
          </Pressable>
        </View>
      )}
    </>
