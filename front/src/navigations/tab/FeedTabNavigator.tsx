...
function TabBarIcons(route: RouteProp<FeedTabParamList>, focused: boolean) {
  ...
  return <Ionicons name={iconName} color={focused ? colors[theme].PINK_700 : colors[theme].GRAY_500} size={25} />;
}
      ...
      <Tab.Screen
        ...
        options={({ route }) => ({
          ...
          tabBarStyle: ((tabRoute) => {
            const routeName = getFocusedRouteNameFromRoute(tabRoute) ?? feedNavigations.FEED_HOME;

            if (routeName === feedNavigations.FEED_DETAIL || routeName === feedNavigations.EDIT_POST) {
              return { display: "none" };
            }
